...
function addListeners (boardNode) {
  for (var i = 0; i < boardNode.children.length; i++) {
    ...
    boardNode.children[i].addEventListener('mouseover', playCellHoverSound)
  }
}
...
function playCellHoverSound() {
  var sound = new Audio("cell_hover.wav")
  sound.play()
}

function playCellMarkedSound() {
  var sound = new Audio("cell_marked.wav")
  sound.play()
}

function playLosingSound() {
  var sound = new Audio("nearby-explosion-with-debris.wav")
  sound.play()
}
